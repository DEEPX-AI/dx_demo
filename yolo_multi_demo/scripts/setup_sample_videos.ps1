...
.SYNOPSIS
    ...
    Downloads and sets up sample video files for yolo_multi_demo.
#>
[CmdletBinding()]
param(
    [string]$SrcPath           = "res/video/sample_videos.tar.gz",
    [string]$Output            = "",
...
if (-not $Output) {
    $Output = Join-Path $SCRIPT_DIR "..\assets\videos"
}
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "        .\scripts\setup_sample_videos.ps1 -Force" -ForegroundColor Yellow
    exit 1
}
